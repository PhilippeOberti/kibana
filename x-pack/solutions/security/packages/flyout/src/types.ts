...
export interface FlyoutApi {
   ...
   */
  openFlyout: (panels: { main?: FlyoutPanelProps; child?: FlyoutPanelProps }) => void;
  /**
   * Replaces the current main panel with a new one
   */
  openMainPanel: (panel: FlyoutPanelProps) => void;
  /**
   * Replaces the current child panel with a new one
   */
  openChildPanel: (panel: FlyoutPanelProps) => void;
  /**
   * Closes main panel
   */
  closeMainPanel: () => void;
