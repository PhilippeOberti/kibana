...
export const useAssistant = ({ alert }: UseAssistantParams): UseAssistantResult => {
  const getPromptContext = useCallback(async () => alert as Record<string, string[]>, [alert]);
  ...
  const { promptContextId, showAssistantOverlay } = useAssistantOverlay(
    CATEGORY,
    conversationTitle,
  ...
  return {
    showAssistant: promptContextId !== null,
