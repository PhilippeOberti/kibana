...
describe('AssistantRowControlColumn', () => {
  ...
  it('should not show the button if assistant is not available', () => {
    (useAssistant as jest.Mock).mockReturnValue({
      showAssistant: false,
      showAssistantOverlay: jest.fn(),
    });
    ...
    const { queryByTestId } = render(<AssistantRowControlColumn alert={alert} />);

    expect(queryByTestId('newChatByTitle')).not.toBeInTheDocument();
  });
});
