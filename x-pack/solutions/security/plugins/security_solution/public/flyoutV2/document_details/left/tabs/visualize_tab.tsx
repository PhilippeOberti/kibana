...
import { useFlyoutState } from '@kbn/flyout';
...
export const VisualizeTab = memo(() => {
  ...
  const [activeVisualizationId, setActiveVisualizationId] = useState(
    panels.child?.path?.subTab ?? SESSION_VIEW_ID
  );
  ...
  useEffect(() => {
    if (panels.child?.path?.subTab) {
      const newId = panels.child.path.subTab;
  ...
  }, [panels.child?.path?.subTab, graphVisualizationEnabled, hasGraphRepresentation]);
